...
router.post('/grava-cadastro-cliente',(req,res)=>{
    ...
    console.log(" origem route : _sample/raq/raq-vendas/grava-cadastro-cliente")
    console.log(' obs : ');
    console.log('');
    console.log(' destino : ');
    console.log('')
...
router.get('/cadfornec/:id',async(req,res)=>{
  ...
  console.log('[ 267 ]',id)
  res.render("_modelos/cad_fornecedor",{layout:'lojista/templete3.handlebars',codClient:id})
})

router.get('/vendas/:id',async(req,res)=>{
      console.log(' { 306 ]')
      console.log('raq/vendas')
      console.log('------------------------')
      let codigoLojista=req.params.id;
      console.log(codigoLojista)
      console.log('------------------------')
      res.render("_cooperado/sample/raqsystem/vendas/vendas-head",{layout:'lojista/templete3.handlebars',codigoLojista:codigoLojista})
})
...
router.get('/listaClientVendas/:id',async(req,res)=>{
  ...
  console.log(' origem route : route/_sample/raq-vendas/listaClientVendas/:id');
  console.log(' Obs : ');
  console.log('');
...
router.get("/clienteHeadrs/:id",async(req,res)=>{
  ...
  console.log('');
  console.log(' origem views : views/_cooperado/sample/raqsystem/vendas.handlebars');
  console.log(' origem route : route/_sample/raq-rotinas/clienteHeadrs/:id');
...
router.get('/produtoRaq/:id',async(req,res)=>{
  ...
  let vendedor=corpo.substring(24,c);
  let nfiscal=0;
  console.log(lojaId);
...
router.post("/edita_quantidade/:id",async(req,res)=>{
  ...
  console.log('_________________________________');
  let IdNota;
  IdNota=corpo.substring(24,48);
  console.log('_id da Nota =>',IdNota)
  /////////////////////////////////////////////////////
...
router.get("/selectProdut/:id",async(req,res)=>{
  ...
  let notaFiscal=restante1.substring(0,pos);
  let vendedor=restante1.substring((pos+4),l)
  ...
  //return
        ...
        Sample.findOne({_id:numberProduto})
              .then((resp)=>{
                 ...
                 console.log(typeof(notaFiscal))
                 const query_nota={
                                   $and:[
                                    {numberNota:notaFiscal},
                                    {vendedor:vendedor},
                                   ]
                 }
                 ...
                 RaqNotaSaida.updateMany(query_nota,insert_Itemnota,options_nota)
                             .then((resultado)=>{
...
router.get('/pegaNotaPendente/:id',async(req,res)=>{
  ...
  let l=palavra.length;
  let nomeVendedor=palavra.substring(0,pos);
...
router.post("/fechamentoNota",async(req,res)=>{
      ...
      console.log(' Obs : acessar relatórios de vendas.');
      console.log('');
      console.log(' destino : abre a views de impressão da nota fiscal.');
      console.log('');
     ...
     async function fecha_nota(valormax){
      ...
      function gravaBoleta(n){
          ...
          /////////////////////////////////////////////////////////////////////////          
          grava_cliente_vendas(n);
          ///////////////////////////////////////////////////////
          function grava_cliente_vendas(valorTot){
                  let operacao1;
                  if (condicao<1){
              ...
              await   RaqFluxo.insertMany
                          (
                            ...
                            .then((result)=>{
                                  ...
                                  /////////////////////////////////////////////////
                                  editaNotaFiscal_condicao(nNota,novaDate,vrParcela);
                                  function editaNotaFiscal_condicao(nNota,novaDate,vrParcela){
                                      RaqNotaSaida.updateMany(
                                                  {numberNota:nNota},
                                                  {
                                                    $push:{
                                                            condicaoPg:[
...
router.get("/buscaPendente/:id",async(req,res)=>{
    ...
    console.log(corpo)
    let pos=corpo.search(":::");
    let vendedor=corpo.substring(0,pos);
    let notafiscal=corpo.substring((pos+3),l);
    console.log('___________________________________________');
    console.log('vendedor',vendedor)
    console.log('numero nf',notafiscal);
    console.log('');
    try{
        RaqNotaSaida.findOne({
                              vendedor:vendedor,
                    ...
                    .then((result)=>{
                      console.log('_____________________________________');
                      console.log('');
                      console.log(' 1904 nFiscal Saída => ',result);
                      console.log('___________________________________________________')
                      let Passa=[];
                      if (result==null){
                           Passa.push(null) 
